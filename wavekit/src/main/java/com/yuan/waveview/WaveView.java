...
public class WaveView extends View {
    ...
    private void init(Context context, AttributeSet attrs) {
        if (attrs != null) {
            //attars
            TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.Y_WaveView);

            int bgColor = typedArray.getColor(R.styleable.Y_WaveView_waveBackgroudColor, BG_COLOR);
            int pColor = typedArray.getColor(R.styleable.Y_WaveView_waveProgressColor, WAVE_COLOR);
            int aMax = typedArray.getInt(R.styleable.Y_WaveView_waveMax, (int) max);
            int aP = typedArray.getInteger(R.styleable.Y_WaveView_waveProgress, (int) progress);
...
//        Log.i(TAG,"onLayout");
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.saveLayer(0, 0, VIEW_WIDTH, VIEW_HEIGHT, null);

        // set shape
        if (mode.equals(MODE_DRAWABLE)) {
            drawableToBitamp(getContext().getResources().getDrawable(R.drawable.wave_love), canvas);
...
//                resetWave();
    ...
    public void onWindowFocusChanged(boolean hasWindowFocus) {
        ...
        //System.out.println("WaveView onWindowFocusChanged hasWindowFocus:"+hasWindowFocus);
...
//        mPaint.setAlpha(200);
    ...
    protected void onDetachedFromWindow() {
       ...
       // System.out.println("WaveView onDetachedFromWindow");
        if (flowingAnimato!=null){
            flowingAnimato.removeAllUpdateListeners();
            flowingAnimato.removeAllListeners();

            flowingAnimato.cancel();
            flowingAnimato=null;
        }
        if (reiseAnimator!=null){
            reiseAnimator.removeAllUpdateListeners();
            reiseAnimator.removeAllListeners();
            reiseAnimator.cancel();
            reiseAnimator=null;
        }
    }

    private void clearFlowing(){
        if (flowingAnimato!=null){
            flowingAnimato.removeAllUpdateListeners();
            flowingAnimato.removeAllListeners();
            flowingAnimato.cancel();
    ...
    public void onDestroy() {
        //System.out.println("WaveView onDestroy");
        clearFlowing();
        clearReise();
    }
